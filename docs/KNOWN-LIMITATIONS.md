...
- The score recognizer is a dedicated digit-cell pipeline behind `DigitRecognizer`, with the documented `TfliteHandwrittenDigitRecognizer` implementation and ML Kit as an explicit fallback. No arbitrary model is bundled: a provenance-backed real-handwriting model still needs to be supplied and evaluated. Blank is based on measured post-border-suppression ink, while ink with failed recognition remains doubtful; corrected page images remain authoritative.
...
- The generic score-sheet path currently preserves the corrected page and OCR evidence as an unidentified record. Deterministic editable table extraction still needs a generic table model and physical calibration data.
