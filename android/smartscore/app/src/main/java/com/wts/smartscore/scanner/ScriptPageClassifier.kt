...
object ScriptPageClassifier {
    const val NEW_SCRIPT_START = "NEW_SCRIPT_START"
    const val CONTINUATION_PAGE = "CONTINUATION_PAGE"
    const val UNCERTAIN_BOUNDARY = "UNCERTAIN_BOUNDARY"
    ...
    fun classify(identity: ScriptIdentity?, ocrText: String): ScriptPageClassification {
        ...
        return when {
            identity != null && identityFields >= 2 && labels >= 2 && headingEvidence >= 2 && score >= 0.58 -> ScriptPageClassification(NEW_SCRIPT_START, score, "labelled identity plus cover heading structure")
            identity != null && (identityFields >= 1 || labels >= 1) && score >= 0.30 -> ScriptPageClassification(UNCERTAIN_BOUNDARY, score, "some identity evidence, but not enough cover structure")
            else -> ScriptPageClassification(CONTINUATION_PAGE, score, "no strong cover boundary evidence")
        }
    }
}
